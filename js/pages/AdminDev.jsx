...
import { Crown, Code2, ShieldCheck, BadgeCheck, ArrowLeft, Search, Ban, Lock, Trash2, Plus, Megaphone, Settings as SetIcon, Wrench, Flag, Upload } from 'lucide-react';
...
export default function AdminDev() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setEmojiPacks(ep || []);
      const { data: rp } = await supabase.from('reports').select('*').order('created_at', { ascending: false }).limit(50);
      if (rp?.length) {
        const ids = Array.from(new Set([...rp.map(r => r.reporter_id), ...rp.map(r => r.target_id)]));
        const { data: ps } = await supabase.from('profiles').select('*').in('id', ids);
        const map = {}; (ps || []).forEach(p => { map[p.id] = p; });
        setReports(rp.map(r => ({ ...r, reporter: map[r.reporter_id], target: map[r.target_id] })));
      }
    })();
  ...
  async function tryAdminLogin(e) {
    e.preventDefault();
    const adminUsername = siteSettings.admin_username || 'admin';
    const adminPassword = siteSettings.admin_password || '1122';
  ...
  async function uploadLogo(e) {
    ...
    try {
      const url = await uploadFile('avatars', f, 'site/');
      setSiteForm(s => ({ ...s, site_logo: url }));
      toast.success('تم رفع الشعار');
    } catch (err) { toast.error(err.message); }
  }
  async function closeReport(id) {
    await supabase.from('reports').update({ status: 'closed' }).eq('id', id);
    setReports(rs => rs.map(r => r.id === id ? { ...r, status: 'closed' } : r));
  }
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-[#0B0C10]">
      ...
      <div className="max-w-6xl mx-auto p-4">
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          <TabBtn id="reports" icon={Flag} label="البلاغات" />
        ...
        {tab === 'reports' && (
          <section className="rounded-2xl p-4 border border-black/5 dark:border-white/10 bg-white dark:bg-white/[0.03]">
            <h3 className="font-semibold mb-3 flex items-center gap-2"><Flag size={14} className="text-rose-500" /> البلاغات</h3>
            <div className="space-y-2">
              {reports.length === 0 && <div className="text-xs text-zinc-400">لا توجد بلاغات.</div>}
              {reports.map(r => (
                <div key={r.id} className="p-3 rounded-xl bg-zinc-50 dark:bg-white/[0.04]" data-testid={`report-${r.id}`}>
                  <div className="text-sm">من <b>@{r.reporter?.username}</b> ضد <b>@{r.target?.username}</b></div>
                  <div className="text-xs text-zinc-500 mt-1">السبب: {r.reason}</div>
                  <div className="text-xs text-zinc-400 mt-1">الحالة: {r.status} · {new Date(r.created_at).toLocaleString()}</div>
                  {r.status === 'open' && <Button size="sm" className="mt-2" onClick={() => closeReport(r.id)}>إغلاق</Button>}
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-2xl p-4 border border-black/5 dark:border-white/10 bg-white dark:bg-white/[0.03] space-y-3">
            ...
            <div>
              ...
              <div className="flex gap-2 items-center">
                {siteForm.site_logo && <img src={siteForm.site_logo} alt="" className="w-12 h-12 rounded-xl object-cover" />}
                <label className="px-3 py-2 rounded-xl bg-zinc-100 dark:bg-white/10 cursor-pointer text-sm flex items-center gap-2"><Upload size={14} /> اختر صورة<input type="file" className="hidden" accept="image/*" onChange={uploadLogo} data-testid="site-logo-upload" /></label>
              </div>
            </div>
            <div>
              <Label>اسم المستخدم للأدمن</Label>
              <Input value={siteForm.admin_username || ''} onChange={(e) => setSiteForm({...siteForm, admin_username: e.target.value})} data-testid="admin-username-setting" />
            </div>
            <div>
              <Label>كلمة المرور للأدمن</Label>
              <Input value={siteForm.admin_password || ''} onChange={(e) => setSiteForm({...siteForm, admin_password: e.target.value})} data-testid="admin-password-setting" />
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl bg-zinc-50 dark:bg-white/[0.04]">
