...
const Home = () => {
  const { settings, buttons, loading } = useSite();
  useEffect(() => { playPage(); }, []);

  // انتظر حتى تنتهي البيانات من التحميل
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center" dir="rtl">
        <div className="text-emerald-400 text-lg animate-pulse">جارٍ التحميل...</div>
      </div>
    );
  }
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-5 py-20 md:py-28 text-center fade-in-up">
            ...
            <div className="relative" style={{ width: size + 40, height: size + 40 }}>
              ...
              <div className="absolute inset-4 rounded-full overflow-hidden border-2 border-emerald-400/40 bg-black">
                {settings.logo_url ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-emerald-300 text-4xl font-bold">
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold leading-tight mb-3">
            ...
            <span
              className="block gradient-text-green mt-2"
              style={{
                background: `linear-gradient(135deg, ${settings.accent_color} 0%, ${primary} 100%)`,
          ...
          {buttons && buttons.length > 0 && (
            <div className="flex flex-wrap items-center justify-center gap-3 mb-2">
              {buttons.map(b => {
                ...
                if (external)
                  return (
                    <a key={b.id} href={b.target_url} target="_blank" rel="noreferrer" onClick={playButton} className="btn-grad px-7 py-3 rounded-full">
                      {b.label}
                    </a>
                  );
                return (
                  <Link key={b.id} to={b.target_url} onClick={playButton} className="btn-grad px-7 py-3 rounded-full">
                    {b.label}
                  </Link>
                );
              })}
