...
export default function Auth() {
  ...
  async function handleSignup(e) {
    ...
    if (!trimmedUsername) { toast.error('اسم المستخدم مطلوب'); return; }
    if (!trimmedDisplayName) { toast.error('الاسم مطلوب'); return; }

    setLoading(true);
    try {
      // Re-check username availability
      const { data: existing } = await supabase.from('profiles').select('username').ilike('username', trimmedUsername).maybeSingle();
      if (existing) {
      ...
      // FIX: Insert profile with the EXACT username and displayName entered by user
      // Use upsert to handle any race condition with AppContext's ensureProfile
          ...
          <div className="rounded-3xl border border-black/5 dark:border-white/10 bg-white/70 dark:bg-white/[0.04] backdrop-blur-xl backdrop-saturate-150 shadow-2xl shadow-black/5 dark:shadow-black/40 p-8">
            ...
            <form onSubmit={mode === 'login' ? handleLogin : handleSignup} className="space-y-4">
                ...
                <>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={username}
                      onChange={(e) => {
                        // Allow dots and underscores in username
                        setUsername(e.target.value.toLowerCase().replace(/\s/g, ''));
                      }}
                      onBlur={(e) => checkUsername(e.target.value)}
                      placeholder="مثال: gaming.mr.riad"
                      required
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full h-11 rounded-xl bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-zinc-100"
