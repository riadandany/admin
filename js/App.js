...
function Protected({ children, allowMaintenanceBypass = false }) {
  const { user, loading, siteSettings, pinUnlocked, setPinUnlocked } = useApp();
  const location = useLocation();

  // Restore PIN session
  useEffect(() => {
    if (sessionStorage.getItem('wave_pin_unlocked') === '1') setPinUnlocked(true);
  ...
  // PIN lock
  const hasPin = !!localStorage.getItem('wave_pin');
  if (hasPin && !pinUnlocked) return <PinLock />;
  ...
  // Maintenance
  if (siteSettings.maintenance && !allowMaintenanceBypass && user.role !== 'owner' && user.role !== 'developer') {
    return <Maintenance />;
  }
...
function RouterContent() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center">...</div>;
  // Public maintenance for unauth visitors
