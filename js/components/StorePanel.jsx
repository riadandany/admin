...
import { X, Coins, Gift, ArrowRightLeft, Sparkles, ChevronDown } from 'lucide-react';

function Category({ title, defaultOpen = false, children, testid }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div className="mb-3 rounded-2xl border border-black/5 dark:border-white/10 overflow-hidden">
      <button onClick={() => setOpen(o => !o)} data-testid={testid} className="w-full p-4 flex items-center justify-between bg-zinc-50 dark:bg-white/[0.04] hover:bg-zinc-100 dark:hover:bg-white/[0.08] transition">
        <h3 className="text-sm uppercase tracking-[0.2em] font-semibold">{title}</h3>
        <ChevronDown size={16} className={`transition ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && <div className="p-4 bg-white dark:bg-white/[0.02]">{children}</div>}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold tracking-tight">إعدادات المتجر</h2>
        ...
        <Category title={T('premiumPlans')} defaultOpen testid="cat-premium">
          <div className="grid sm:grid-cols-3 gap-3">
            {items.filter(i => i.type.startsWith('premium_')).map(i => <ItemCard key={i.id} item={i} />)}
            {items.filter(i => i.type.startsWith('premium_')).length === 0 && <div className="text-xs text-zinc-400">لا توجد عناصر.</div>}
          </div>
        </Category>

        <Category title={T('frames')} testid="cat-frames">
          <div className="grid sm:grid-cols-4 gap-3">
            {items.filter(i => i.type === 'frame').map(i => <ItemCard key={i.id} item={i} />)}
            {items.filter(i => i.type === 'frame').length === 0 && <div className="text-xs text-zinc-400">لا توجد عناصر.</div>}
          </div>
        </Category>

        <Category title={T('nameColors')} testid="cat-colors">
          <div className="grid sm:grid-cols-3 gap-3">
            {items.filter(i => i.type === 'name_color').map(i => <ItemCard key={i.id} item={i} />)}
            {items.filter(i => i.type === 'name_color').length === 0 && <div className="text-xs text-zinc-400">لا توجد عناصر.</div>}
          </div>
        </Category>

        <Category title={T('backgrounds')} testid="cat-backgrounds">
          <div className="grid sm:grid-cols-3 gap-3">
            {items.filter(i => ['background_color','background_image','group_member_slot'].includes(i.type)).map(i => <ItemCard key={i.id} item={i} />)}
            {items.filter(i => ['background_color','background_image','group_member_slot'].includes(i.type)).length === 0 && <div className="text-xs text-zinc-400">لا توجد عناصر.</div>}
          </div>
        </Category>

        <Category title={T('emojiPacks')} testid="cat-emojis">
          <div className="grid sm:grid-cols-3 gap-3">
            ...
            {emojiPacks.filter(p => p.locked).length === 0 && <div className="text-xs text-zinc-400">لا توجد حزم.</div>}
          </div>
        </Category>
      </motion.div>
    </div>
