...
const AdminMessages = () => {
  ...
  const remove = async (id) => { if (!confirm('حذف الرسالة؟')) return; await supabase.from('messages').delete().eq('id', id); load(); };
  return (
    <div>
        ...
        <div className="space-y-3">{msgs.map(m => (
          <div key={m.id} className={`glass-card rounded-xl p-5 ${!m.is_read ? 'border-r-4 border-r-emerald-400' : ''}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={()=>remove(m.id)} className="text-xs px-3 py-1.5 rounded-lg bg-red-500/10 text-red-300 border border-red-500/30 hover:bg-red-500/20 transition flex items-center gap-1"><Trash2 size={12}/>حذف</button>
            </div>
          </div>
        ))}</div>
      )}
