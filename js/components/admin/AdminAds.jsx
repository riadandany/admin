        ...
        <div className="space-y-3">
          ...
          <div><label className="label-light">وضع العرض</label>
            <select className="input-dark" value={a.display_mode||'once'} onChange={(e)=>setA({...a, display_mode: e.target.value})}>
              <option value="once">مرة واحدة (نهائيًا)</option>
              <option value="session">مرة واحدة لكل جلسة</option>
              <option value="always">في كل زيارة</option>
            </select></div>
          <div><label className="label-light">الصفحات التي تظهر فيها</label>
...
const AdminAds = () => {
  ...
  const remove = async () => {
    ...
    await supabase.from('ads').delete().eq('id', confirmDelete.id);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={() => setEditing({title:'', show_on_pages:['*'], display_mode:'once', is_active:true})} className="btn-grad px-4 py-2 rounded-lg text-sm flex items-center gap-1.5"><Plus size={16}/>إضافة إعلان</button>
      </div>
      <div className="space-y-3">
        ...
        {ads.map(a => (
          <div key={a.id} className="glass-card rounded-xl p-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setConfirmDelete({ id: a.id, title: a.title })} className="px-3 py-2 rounded-lg bg-red-500/10 text-red-300 border border-red-500/30 hover:bg-red-500/20 transition"><Trash2 size={14}/></button>
            </div>
          </div>
        ))}
      </div>
      ...
      <ConfirmDialog
        open={!!confirmDelete}
        message={`هل تريد حذف الإعلان "${confirmDelete?.title}"؟`}
        onConfirm={remove}
